...
ndarr = np.empty((691,886,1), dtype=np.float32)
...
def main():
  ...
  parser.add_argument("start", help="starting date YYYYMMDD of the search range")
  ...
  datelist = pd.to_datetime(pd.date_range(start=start, end=end, freq='D').tolist())
  days_history = (end - start).days + 1

  # load the 3D Array for a Year's History
  global ndarr
  ndarr = np.empty((691,886,days_history), dtype=np.float32)

...
gda94 = proj.Proj(init='epsg:3112')


def pixel_coords_from_map_coords(lng, lat):
    x, y = gda94(lng, lat)
...
def construct_filename(prefix, suffix, start):
...
def check_cache(filepath, d):
    f = filepath + construct_filename(d)
...
def download_from_bom(url, path, savefile, date):
    """Downloads and extracts data from BOM based on date range"""
    inputfile = bom_filename(date)
...
def dfmc_retrieve_data_for_date(d):
    ...
    for params in parameters:
        ...
        archive_file = construct_filename(
            params['prefix'], suffix + compression_suffix, d)
        grid_file = construct_filename(params['prefix'], suffix, d)
        ...
        if not os.path.isfile(params['path'] + grid_file):
            ...
            if not os.path.isfile(params['path'] + archive_file):
                ...
                download_from_bom(params['url'], params['path'], archive_file, d)
            # else:
            # Yes - exists.
            # print('>> Archived Data for ' + d.strftime("%Y%m%d") + ' already exists in ' + params['path'] + '. Not downloading.')

        # Now expand it...
        expand_archive_file(params['path'], archive_file)
    # else:
...
def dfmc_calculate(date):
    DFMC_filename = construct_filename(DFMC_prefix, '.grid', date)
    P_filename = construct_filename(precip_prefix, '.grid', date)

    # load the file using rasterio
    VP3pm_filename = construct_filename(vapour_prefix, suffix, date)
    ...
    Tmx_filename = construct_filename(temp_prefix, '.grid', date)
...
def dfmc_time_series_at_point(lng, lat, datelist):
    tolerance = 0.06
    v = -999999
    vmin = -999999
    vmax = -999999
    ...
    x, y = pixel_coords_from_map_coords(lng, lat)
    ...
    if ndarr[0, 0, 0] > 0:
        ...
        output = pool.map(dfmc_on_date, datelist)
...
def dfmc_complete_map_for_date(date):
    # STUB!
    return None


def dfmc_on_date(date):
...
def dfmc_at_point_on_date(location, date):
    """With the date appropriate file in the Tmx, VP3pm, and P folder: \
    calculate the DFMC and precipitation at a specific lat/lng"""
...
def write_net_cdf4(data):
    ...
    tempgrp = f.createGroup('/DFMC_data')  # in Root Group ie., "/"
    ...
    f.description = "Example dataset containing group moisture values by Dead Fuel Moisture Content Model Nolan et al."
...
def main():
    ...
    print(dumps(dfmc_time_series_at_point(lng, lat, datelist)))
